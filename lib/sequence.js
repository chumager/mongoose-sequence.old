 ...
 */
Sequence = function(schema, options, mongoose) {
  var defaults = {
    id: null,
    inc_field: "_id",
  ...
  this.sequenceArchive = SequenceArchive.getSingleton(mongoose);
 ...
 *
 * @method     getInstance
 * @param      {Object}    mongoose   A mongoose Schema
 *                                  constructor
 * @return     {Sequence}  A sequence generator
 *
 * @static
 */
Sequence.getInstance = function(mongoose){
  return function(schema, options) {
    const sequence = new Sequence(schema, options, mongoose),
      id = sequence.getId();


    if (sequence.sequenceArchive.existsSequence(id)){
      throw new Error("Counter already defined for field \""+id+"\"");
    }
    sequence.enable();
    sequence.sequenceArchive.addSequence(id, sequence);
    return sequence;
  };
};
...
Sequence.prototype._createCounterModel = function() {
  ...
  CounterSchema = this.mongoose.Schema(
    {
      id: {type: String, required: true},
      reference_value: { type: this.mongoose.Schema.Types.Mixed, required: true},
  ...
  return this.mongoose.model("Counter_" + this._options.id, CounterSchema);
};
...
Sequence.prototype._setMethods = function() {
  ...
  this._schema.method("setNext", function(id, callback) {
    var sequence = this.sequenceArchive.getSequence(id);
...
module.exports = function(mongoose){
  ...
  return Sequence.getInstance(mongoose);
};
